...
const insertValuesIntoTable = async () => {
    const sqlBeginning = `INSERT INTO ship_types (ship_class_id, ship_class_name) VALUES`
    let longString = ''
    for(let i = 1; i < keys.length; i++){
        if(i === keys.length - 1){
            longString += ` (${keys[i]}, '${values[i]}');`;
        } else {
            longString += ` (${keys[i]}, '${values[i]}'),`;
        }
    }
    const sql = sqlBeginning + longString;
    client.connect();
    client.query(sql, (err, res) => {
        if(err){
            ...
            console.log('hrrrng... done');
        }
    })
}

module.exports = insertValuesIntoTable;
