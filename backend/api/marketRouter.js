...
//send market data for a specific subsystem + mats cost
marketRouter.get(`/:subsystemID`, (req, res, next) => {
    if (!req.params.subsystemID || !subsystemIDArr.includes(req.params.subsystemID)) {
        res.status(404).send();
        return;
    } else {
        const id = req.params.subsystemID;
        const subsystemType = subsystemObjects.find(subsystem => subsystem.id == Number(id));
        let type = "";
        if (subsystemType.name.includes("Core")) {
            type = "core";
        }
        if (subsystemType.name.includes("Defensive")) {
            type = "defensive";
        }
        if (subsystemType.name.includes("Propulsion")) {
            type = "propulsion";
        }
        if (subsystemType.name.includes("Offensive")) {
            type = "offensive";
        }

        client.query(`SELECT * FROM subsystem_costs WHERE subsystem_type = '${type}' AND region = 'jita' ORDER BY date ASC`)
            .then((response) => {
                output.subsystemCosts = response.rows;
            })
            .catch(err => console.log(err))

        client.query(`SELECT * FROM price_data WHERE type_id = '${id}' AND region = '10000002' ORDER BY date ASC`)
            .then((response) => {
...
marketRouter.get(`/:subsystemID`, (req, res) => {
        ...
        .then((res) => {
            ...
            res.rows.map((row) => {
                if (!pieChartData[row.type_id]) {
                    ...
                    pieChartData[row.type_id].name = row.type_name;
                } else {
                    pieChartData[row.type_id].value += 1;
                }
            })
            output.pieChartData = pieChartData;
